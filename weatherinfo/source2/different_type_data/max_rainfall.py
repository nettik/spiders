...
def get_max_rainfall(station_id):
    result = list()
...
def main_max_rainfall():
    ...
    while not task_queue.empty():
        ...
        if task["type"] == "province_code":
            ...
            ret = get_max_rainfall(task["data"])
            time.sleep(0.5)
            if ret is not None:
                rainfall_t = [task["name"]]
                for item in ret:
                    rainfall_t.append(item["data"])
                max_rainfall.append(rainfall_t)
            else:
                task["count"] += 1
                task_queue.put(task)
    save_in_file(max_rainfall)
