...
def get_average_humidity(station_id):
    result = list()
...
def main_average_humidity():
    ...
    while not task_queue.empty():
        ...
        if task["type"] == "province_code":
            ...
            ret = get_average_humidity(task["data"])
            time.sleep(0.5)
            if ret is not None:
                rainfall_t = [task["name"]]
                for item in ret:
                    rainfall_t.append(item["data"])
                average_humidity.append(rainfall_t)
            else:
                task["count"] += 1
                task_queue.put(task)
    save_in_file(average_humidity)
