...
const askbPGkwehuv = "a90D4t8oimimP2rE";
const asiEQuflquNk = "bSd37NjnO1QKa5gy";
const acke1zNeduHV = "duQsKXoLW1PfUFMS";
const aci4Acudl9PQ = "fvJF8wfuVi0Duqmt";
const dsk4kfMmNqyH = "hw4x1BV74k0uCf1R";
const dsishLShSFfi = "xuXTpujwdBEDVPSs";
const dck28xLxVgKV = "o4vqDe9vOF0u1NCt";
const dciOOF7xfhPU = "prB5bsWYQBXuOFMI";
...
function gzxUwEPNXMkdkZ3i(key, period) {
...
function dCHpZnByBSf0oOR(data) {
    data = BASE64.decrypt(data);
    data = DES.decrypt(data, dsk4kfMmNqyH, dsishLShSFfi);
    data = AES.decrypt(data, askbPGkwehuv, asiEQuflquNk);
    data = BASE64.decrypt(data);
    return data
}
var poDneQXn7ugw = (function () {
    function osLNCpgUrV(obj) {
        var newObject = {};
        Object.keys(obj).sort().map(function (key) {
            newObject[key] = obj[key]
    ...
    return function (mvCetEJYZ, oHjS4g) {
        var aCVf = '373f1f736aacd97fd4cfe50b0135d88f';
        var c4mh3 = 'WEB';
        var tDO3k1u = new Date().getTime();
        var p1r45Id = {
            appId: aCVf,
            method: mvCetEJYZ,
            timestamp: tDO3k1u,
            clienttype: c4mh3,
            object: oHjS4g,
            secret: hex_md5(aCVf + mvCetEJYZ + tDO3k1u + c4mh3 + JSON.stringify(osLNCpgUrV(oHjS4g)))
        };
        p1r45Id = BASE64.encrypt(JSON.stringify(p1r45Id));
        p1r45Id = AES.encrypt(p1r45Id, acke1zNeduHV, aci4Acudl9PQ);
        return p1r45Id
    }
})();

function sUxfxuNU7tGPQHkFp(mvCetEJYZ, oZNn7J0s49, c0QHGwvND, pJmY5na) {
    const kpKy = hex_md5(mvCetEJYZ + JSON.stringify(oZNn7J0s49));
    const d7ZIS = gzxUwEPNXMkdkZ3i(kpKy, pJmY5na);
    if (!d7ZIS) {
        var p1r45Id = poDneQXn7ugw(mvCetEJYZ, oZNn7J0s49);
        $.ajax({
            ...
            type: "post",
            success: function (d7ZIS) {
                d7ZIS = dCHpZnByBSf0oOR(d7ZIS);
                oHjS4g = JSON.parse(d7ZIS);
                if (oHjS4g.success) {
                    if (pJmY5na > 0) {
                        oHjS4g.result.time = new Date().getTime();
                        localStorageUtil.save(kpKy, oHjS4g.result)
                    }
                    c0QHGwvND(oHjS4g.result)
                } else {
                    console.log(oHjS4g.errcode, oHjS4g.errmsg)
                }
            }
        })
    } else {
...
function getEncryptedData(method, city, month) {
    ...
    param.city = city;
    param.month = month;
    return poDneQXn7ugw(method, param);
}